...
void TrackKLT::perform_matching(const std::vector<cv::Mat> &img0pyr, const std::vector<cv::Mat> &img1pyr, std::vector<cv::KeyPoint> &kpts0,
  ...
  // TODO: Elegantly handle the previous frame descriptor
  cv::Mat prevDescriptors, currDescriptors;
  if (!this->ORBDescriptor->computeDescriptors(img0pyr.at(0), pts0, prevDescriptors) ||
      !this->ORBDescriptor->computeDescriptors(img1pyr.at(0), pts1, currDescriptors)) {
  ...
  for (size_t i = 0; i < mask_klt.size(); i++) {
    auto mask =
        (uchar)((i < mask_klt.size() && mask_klt[i] && i < mask_rsc.size() && mask_rsc[i] && i < mask_desc.size() && mask_desc[i]) ? 1 : 0);
  ...
  if (false) {
    ...
    for (size_t i = 0; i < pts0.size(); i++) {
      ...
      if (!mask_desc[i]) {
        ...
        int dis = AMCVIO::ORBdescriptor::computeDescriptorDistance(prevDescriptors.row(i), currDescriptors.row(i));
        cv::putText(img_desc, std::to_string(dis), (pts0.at(i) + line_end) / 2, cv::FONT_HERSHEY_COMPLEX_SMALL, 1, cv::Scalar(255, 0, 255),
                    1);
      }
    }
